...
export default function DonationComments({ donations_id }) {
    ...
    if (currentUser) {
      ...
      try {
        const response = await axios.post(
          `${API}/donation-comments`,
          actualNewDonationComment
        );
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${API}/donations/${donations_id}/donation-comments`
        );
        console.log(response.data);
  ...
  return (
    <div>
      <button
        className="button is-rounded is-primary has-text-weight-bold mt-3"
      ...
      <div>
        {isExpanded && (
          <div className="comments-box has-background-success px-3 py-3">
            ...
            <ul className="has-text-dark py-5">
